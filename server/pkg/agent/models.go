...
// "Currently supported:" model list from the --model flag description.
// CodeBuddy Code does not have a dedicated `models` or `--list-models`
// subcommand; the canonical catalog lives in the CLI help text and is
// always accurate for the installed version.
//
// Falls back to codebuddyStaticModels() when the binary is missing or
// when parsing fails, so the UI dropdown always has something to show.
func discoverCodeBuddyModels(ctx context.Context, executablePath string) ([]Model, error) {
	...
	models := parseCodeBuddyModelsFromHelp(string(out))
	if len(models) == 0 {
		return nil, nil
	}
	return models, nil
...
// codebuddyModelHelpRe matches the model list in the --help output:
//
//	--model <model>  Model for the current session. Please provide the
//	model ID. Currently supported: (claude-sonnet-4.6,
//	claude-sonnet-4.6-1m, claude-opus-4.8, ..., deepseek-v3-2-volc-ioa,
//	custom-local:deepseek-v4-pro, custom-local:deepseek-v4-flash)
var codebuddyModelHelpRe = regexp.MustCompile(`Currently supported:\s*\(([^)]+)\)`)

// parseCodeBuddyModelsFromHelp extracts model IDs from `codebuddy --help`
// output. Model IDs are comma-separated inside the parentheses following
// "Currently supported:".
func parseCodeBuddyModelsFromHelp(output string) []Model {
	m := codebuddyModelHelpRe.FindStringSubmatch(output)
	if m == nil {
		return nil
	}
	...
	// Known vendor prefixes used to assign Provider for UI grouping.
	vendorPrefixes := []struct {
		prefix   string
		provider string
	}{
		{"claude-", "anthropic"},
	...
	for _, raw := range ids {
		...
		// Derive a human-readable label from the model ID.
	...
	// If there's exactly one model or if the first model is the user's
	// configured default, mark it as Default. Otherwise leave Default
	// empty so the UI doesn't badge any entry — the runtime picks its own
	// default when agent.model is empty.
	if len(models) > 0 {
...
// modelIDToLabel converts a model ID like "claude-sonnet-4.6" or
// "deepseek-v4-pro-ioa" into a human-readable label by applying simple
// capitalization and spacing heuristics.
func modelIDToLabel(id string) string {
	// Strip vendor-specific suffixes that are internal routing markers.
	id = strings.TrimSuffix(id, "-ioa")
	id = strings.TrimSuffix(id, "-volc")

	// Capitalize known vendor names.
	id = strings.ReplaceAll(id, "claude", "Claude")
	id = strings.ReplaceAll(id, "gemini", "Gemini")
	id = strings.ReplaceAll(id, "gpt", "GPT")
	id = strings.ReplaceAll(id, "deepseek", "DeepSeek")
	id = strings.ReplaceAll(id, "glm", "GLM")
	id = strings.ReplaceAll(id, "minimax", "MiniMax")
	id = strings.ReplaceAll(id, "kimi", "Kimi")
	id = strings.ReplaceAll(id, "hunyuan", "Hunyuan")

	return id
}

// ── CodeBuddy static fallback catalog ──

// codebuddyStaticModels provides a minimal fallback model list when the
// CLI is not available on the daemon host. The real catalog is discovered
// dynamically via discoverCodeBuddyModels() which parses `codebuddy --help`.
// Keep this list small and generic — the dynamic path is the primary source
// of truth.
func codebuddyStaticModels() []Model {
	return []Model{
		...
		{ID: "deepseek-v4-pro-ioa", Label: "DeepSeek V4 Pro", Provider: "deepseek"},
		{ID: "deepseek-v4-flash-ioa", Label: "DeepSeek V4 Flash", Provider: "deepseek"},
